...
function previewSheetKit(input) {
  ...
  var record = {version: 3, spreadsheetId: SpreadsheetApp.getActiveSpreadsheet().getId(), sheetId: range.getSheet().getSheetId(), a1: range.getA1Notation(), digest: digest_(range), options: options};
...
function applySheetKit(token) {
  ...
  try {
    ...
    if (record.version !== 3) throw new Error('SheetKit was updated. Close the sidebar, reopen it and preview again.');
    if (record.spreadsheetId !== ss.getId()) throw new Error('This preview belongs to another spreadsheet. Preview again here.');
